...
def create_local_nodes_dict(node,engine,table = 'local_node_distances',graph=G, distance = 3200):
    # Calculate node relations, for subset of nodes leading to destinations
    print(len(node))
    # d[f'local_{distance}m'] = d.node.apply(lambda x: nx.multi_source_dijkstra_path_length(graph, {x}, cutoff=distance, weight='length'))
...
if __name__ == '__main__':
    ...
    def parallel_lapply_function(id_list, func, chunks = 10, n_cores=5):
        ...
        pool.map(func, id_list, chunk_size)
    ...
    df = parallel_lapply_function(distinct_destination_nodes,create_local_nodes_dict)
