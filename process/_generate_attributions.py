...
def main():
    ...
    df = df_datasets.query("role in ['indicators','destinations','methods','boundaries']").copy().sort_index()
    ...
    df = df[['role','resource', 'attribution','dimension', 'category', 'type', 'region','data_name','method_description_data','method_description_ind','data_file','source_url','date_acquired','licence','licence_url']] \
        .drop_duplicates() \
        .sort_values(['role','dimension','category','resource'])
    df[['role','resource','attribution']].to_csv(f'{output_dir}/csv/attribution_{date}_brief.csv',index=False)
