...
def main():
    ...
    engine = create_engine("postgresql://{user}:{pwd}@{host}/{db}".format(user = db_user,
                                                                          ...
                                                                          host = db_host,
                                                                          db   = db))      
    curs.execute('''SELECT 1 WHERE to_regclass('public.{}') IS NOT NULL;'''.format(hex_grid))
    res = curs.fetchone()
    if res is None:
    ...
    curs.execute('''SELECT 1 WHERE to_regclass('public.{}') IS NOT NULL;'''.format(hex_grid_buffer))
    res = curs.fetchone()
    if res is None:   
        sql = '''
