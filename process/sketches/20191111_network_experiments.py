...
def parallelize_dataframe(df, func, chunk_size = 10000, n_cores=6):
    start_time = time.time()
    nrows = len(df)
    chunks = int(np.ceil(nrows / float(chunk_size)))
...
def create_local_nodes_dict(d,graph=G, distance = 3200):
